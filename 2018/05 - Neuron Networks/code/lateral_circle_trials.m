...
counts_over_trials = zeros(10,n_neurons);
for trials=1:10
  ...
  counts_over_trials(trials,:) = sum(spikes, axis=1);
end

disp("Average counts of spikes for different positions in circular condition are:")
disp(sum(counts_over_trials,axis=1)/10.0)

plot(sum(counts_over_trials,axis=1)/10.0)
xlim([0.9,10.1])
ylim([0,25])

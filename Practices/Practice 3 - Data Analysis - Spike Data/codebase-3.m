...
% and specify font size for number on the axis
set(gca,'FontSize', 14)
...
%% Exercise 2
% in this exercise you should produce two raster plots

% first one will have time on X axis and 72 neurons on Y axis for one
...
% to average we add all trials together
...
% matrix column-wise givin us one vector of lenght 1501
spikes = sum(spikes)

% because we summed everything together it might happen, that in two trial
% the spike occured on the same time moment, this will result in having
% values 2 (or gratear) in our final vector, which is bad, bacause other
...
% plot, each neuron on its own line, this is very similar to the Exercise 1
%
...
%% Exercise 3
% for that one you will need use hist() or bar() function to produce a plot
% otherwise is also should be somewhat similar to previous ones
...
%% Exercise 4
% read about the concept of the tuning curve in the exercise sheet
% the operation you will need to perform are, again, similar to the ones
% you have seen

% at some point you will need to compute standard deviation
deviation = std([1 2 1 1 2 1 2 1 2 1 2]);

% and user the special kind of plot, which plots error bars on top of the
% curve, which expects you to give two vectors as an input: data points and
% deviation (or error) for each of the points
curve = [1 3 4 2 4 5 2 3 4 5 2 3 3 4];
deviations = repmat(deviation, 1, size(curve, 2));
errorbar(curve, deviations)
...
%% Exercise 6
% by now you know bunch of tools and you can use them to tackle this
% problem, which is, I must say, an example of simple, but real scientific question
